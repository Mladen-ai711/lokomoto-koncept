# v12 — nadogradnja v11

Nastavak na v11. Menjaju se: sekcija 02 USLUGE, nova traka brojeva ispod heroja,
i blok TIM unutar sekcije 03.

## Šta je novo

### 1. Usluge (sekcija 02)
- Naslov: „Sve što je potrebno za promišljen oporavak. **Bez lutanja.**"
- Podnaslov: „Od prvog pregleda do povratka punoj funkciji. Ne birate vi terapiju — biramo je zajedno, posle pregleda."
- Preview panel dobija liniju „za koga je" (menja se sa uslugom)
- Preview panel dobija tri čipa: trajanje · format · svrha
- Meki CTA ispod liste: „Ne znate koja vam usluga treba? Ne morate da znate." → #kontakt
- Osveženi opisi svih 6 usluga

### 2. Sa čime nam dolaze (novo, u sekciji 02 iznad liste usluga)
Dvanaest tegoba pacijentovim jezikom, kao čipovi. Klik na čip otvara
odgovarajuću uslugu u listi ispod, skroluje do nje i nakratko je istakne.

### 3. Traka brojeva (novo, odmah ispod heroja)
Tamna traka između heroja i lime trake sa ciljevima. Četiri podatka:
4.000+ tretmana godišnje · 6 fizioterapeuta · 1:1 terapeut po pacijentu · 48h do prvog termina

### 4. Tim (novo, na dnu sekcije 03)
- Naslov: „Iza svakog plana stoji **neko ko ga vodi.**"
- Šest kartica: Novak Ilić sa pravom fotografijom + pet sa monogramom
- Raspored 4 + 2; drugi red popunjava blok „Prosečno 8 godina kliničkog iskustva po terapeutu"

### 5. Česta pitanja (novo, blok bez broja pre sekcije 05)
Deset pitanja u harmonici — otvara se jedno po jedno. Pokriva i prvi pregled,
pa zasebna sekcija „Kako izgleda prvi pregled" više nije potrebna.

### 6. Futer (prerađen)
Četiri kolone: brend + mreže / Sajt / Usluge / Kontakt sa radnim vremenom.
Dole traka sa godinom, politikom privatnosti i „na vrh".

## Zavisnosti
Ovaj folder NE sadrži slike, video ni fontove — koristi postojeće iz `../v8/assets/`
i logo/favicon iz `../v11/`. Ništa drugo ne treba uploadovati.

## PLACEHOLDER PODACI — obavezno proveriti pre objave

**Traka brojeva** — sva četiri broja su izmišljena:
- 4.000+ tretmana godišnje
- 6 fizioterapeuta u timu
- 1:1 terapeut po pacijentu (proveriti da li je zaista uvek tako)
- 48h prosečno do prvog termina

**Tim** — pet od šest kartica je prazan šablon:
- „Ime Prezime" i monogram „IP" MORAJU se zameniti pravim imenima i fotografijama
- Uloge (fizijatar / fizioterapeut / kineziterapeut) su pretpostavka
- Novak Ilić: proveriti tačnu titulu i opis specijalnosti
- „Prosečno 8 godina kliničkog iskustva" je izmišljen podatak

**Tegobe** — spisak i povezivanje sa uslugama su moja pretpostavka.
Klijent treba da potvrdi: da li su to zaista najčešći razlozi dolaska, i da li
svaka tegoba vodi na pravu uslugu (npr. „Ukočen vrat i glavobolje" → Manualna terapija).

**Futer** — tri stvari su placeholder:
- Radno vreme (Pon–Pet 08–20, Sub 09–14, Ned ne radimo) — izmišljeno
- Instagram i Facebook vode na `#` — zameniti pravim adresama ili obrisati ikonice
- „Politika privatnosti" vodi na `#` — treba stranica ili link

**FAQ** — tri odgovora imaju rupe koje samo klijent može da popuni:
- „Koliko košta?" → `[XXX]` na dva mesta
- „Radite li preko RFZO?" → ceo odgovor je placeholder
- „Gde ste i ima li parkinga?" → deo o parkingu
Ostalih sedam odgovora su moja pretpostavka — proveriti brojeve (45 min, 4–6
dolazaka, otkazivanje 24h) pre objave.

**Usluge** — trajanja su pretpostavka:
45 min (dijagnostika), 30–40 (fizikalna), 45–60 (kinezi), 30 (manualna), 60 (medicinski trening).
Takođe proveriti „Saradnja sa hirurgom" kod postoperativne rehabilitacije.
