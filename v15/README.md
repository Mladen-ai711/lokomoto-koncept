...
# v15 — samostalan folder i isečci panela (13.09.2026)

v14 nije diran. v15 je njegova kopija sa dve promene: resursi su uneti unutra, i
tri panela koja su ostala sa pogrešnim odnosom slike dobila su isečke.

## 1. v15 više ne vuče ništa spolja

v14 je povlačio CSS, favicon, fontove, slike i video iz `v8/`, `v11/` i `v12/`,
na tri različite dubine (`../`, `../../`, `../../../`). Na produkcijskom deployu,
gde tih foldera nema, sajt se učitavao bez stila i bez ijedne slike.

**Prepisane 73 putanje u 9 fajlova.** Pravilo: skine se tačno jedan `../`, a
`v8/assets/` → `assets/`, `v12/styles.css` → `styles.css`,
`v11/favicon.svg` → `favicon.svg`.

| Šta je ušlo u `v15/assets/` | Broj |
|---|---|
| `images/` | 24 |
| `media/` (hero video, desktop i mobilni) | 2 |
| `fonts/` (Instrument Sans) | 4 + licenca |

Uz to `v15/styles.css` (kopija `v12/styles.css`) i `v15/favicon.svg`.

**Fontove je uhvatio render, ne statička provera.** Prvi prolaz provere tražio je
`webp|jpg|png|svg|css|js|mp4` i javio 81 ispravan resurs i 0 puknutih — ali
`.woff2` nije bio u spisku, pa je Instrument Sans ostao da pokazuje u `v8/`.
Chromium je vratio četiri 404. Zato provera nije završena dok stranica nije
otvorena u pregledaču.

Provereno posle ispravke: 1440 i 1000 px, bez ijedne 404 i bez JS greške.

## 2. Paneli — tri su ostala sa pogrešnim odnosom

Zatečeno stanje, mereno:

| Panel | Slika | Dimenzije | Odnos | Stanje |
|---|---|---|---|---|
| Dijagnostika | `assessment.webp` | 1280×1600 | **0,80** | uspravna u položenoj karti |
| Fizikalna terapija | `panel-fizikalna.webp` | 1600×1200 | 1,33 | rešeno ranije |
| Kineziterapija | `panel-kineziterapija.webp` | 1600×1200 | 1,33 | rešeno ranije |
| Manualna terapija | `system-detail.webp` | 900×720 | 1,25 | **ostavljeno, v. niže** |
| Postoperativna | `manifesto.webp` | 1280×720 | **1,78** | preširoka za uspravnu kartu |

Karta panela menja oblik sa širinom ekrana — 669×752 (**0,89**) na 1440 px i
920×608 (**1,51**) na 1000 px — pa jedna slika mora da posluži oba. Cilj ostaje
4:3, iz istog razloga kao i prošli put.

### Izbor isečka: merenje po ivičnoj energiji je promašilo, oba puta

Prvo je za svaki izvor izračunat prozor 4:3 koji zadržava najviše ivične energije:

| Izvor | „Najbolji" pomak | Zadržano |
|---|---|---|
| `assessment.webp` | y = 85 | 72,6 % |
| `manifesto.webp` | x = 320 | 78,7 % |

**Oba su odbačena na pogledu.** Ivična energija nagrađuje mesto gde ima *detalja*,
a ne gde je *radnja*:

- `assessment` na y=85 seče šake na donjoj ivici — ostaju dva lica bez pregleda.
- `manifesto` na x=320 izbacuje ultrazvučni aparat sa leve strane, a dobija praznu
  stolicu i sudoperu sa desne.

Uzeto je, sa liste od tri kandidata po slici:

| Fajl | Izvor | Isečak | Zašto |
|---|---|---|---|
| `panel-dijagnostika.webp` | `assessment` | **y = 200..1160** | jedini kadar sa oba lica *i* šakama na ramenu |
| `panel-postoperativna.webp` | `manifesto` | **x = 70..1030** | subjekt centriran, aparat ostaje u kadru |

| Novi fajl | Dimenzije | Odnos | Veličina |
|---|---|---|---|
| `panel-dijagnostika.webp` | 1280×960 | 1,33 | 64 KB |
| `panel-postoperativna.webp` | 960×720 | 1,33 | 34 KB |

Nijedna slika nije uvećavana — oba isečka su u izvornoj rezoluciji.

### Zašto manualna terapija nije dirana

`system-detail.webp` je 900×720, odnos **1,25**. Do cilja 1,33 nedostaje 6 %, a
sečenje na 4:3 dalo bi 900×675 — kartica je široka 920 px, pa bi se slika time
spustila ispod svoje širine bez dobitka u kompoziciji. Kompozicija je već dobra:
Ergon alat na ramenu, subjekt od ivice do ivice.

**Otvoreno:** njena stvarna granica je rezolucija (900 px na kartici od 920 px),
i to se rešava samo povratkom na original.

## 3. Šta ovo ne rešava

Za ove tri stvari treba folder `E:\Lokomoto` sa 104 kadra, kog u repou nema:

- **Iste slike se ponavljaju preko stranica usluga.** Slika uz korake na
  dijagnostici i dalje ne prikazuje ultrazvuk. Rešenje su kadrovi `L-30`–`L-32`.
- **Manualna terapija** bi dobila pravu sliku iz `L-33`–`L-46`, umesto deljene.
- **Kartice metoda** za magnetoterapiju, krioterapiju i limfnu drenažu — potvrđeno
  ranije da tih kadrova nema ni na jednom od 104 snimka.
...
1. **Okvir hera na telefonu je oko 0,46, nije 9:16.** Poster — a i video, ako
   se ikad vrati — mora da prati taj odnos. Ako ne prati, `object-fit: cover`
   odseca još **18% širine** pre nego što iko pogleda. **To je bio pravi uzrok
   „makro" prijave, a ne rez.**
